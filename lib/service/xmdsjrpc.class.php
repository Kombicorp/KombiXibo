...
//        syslog( LOG_INFO, 'XMDSJsonRpc construct');
//    }

//    public function __construct()
//    {
//
//      syslog(LOG_INFO, "XMDSJsonRpc()");
      //parse and init the settings.xml
//      Config::Load();
      // gregm added
//      syslog(LOG_INFO, "XMDSJsonRpc() after Config::Load()");
...
//    }
    ...
    function simpleTest($client) {
      return 'hey '.$client.'! You are asking for it!';
	...
	*/
     ...
     * @return <type>
     */
    function registerDisplay($serverKey, $hardwareKey, $displayName, $clientType, $clientVersion, $clientCode, $operatingSystem, $macAddress)
    {
//	$jsonResult = (object)[];
...
//        $clientVersion = Kit::ValidateParam($clientVersion, _STRING);
        syslog(LOG_INFO, 'RegisterDisplay : '.$clientVersion);
...
//        $sk = '42pzE9';
        syslog(LOG_INFO, 'RegisterDisplay - sk : '.$sk);

       	$jsonResult = new stdClass();
//	class $jsonResult {};
//    if ($serverKey != Config::GetSetting('SERVER_KEY')) {
      if ($serverKey != $sk) {
	  syslog(LOG_INFO, 'RegisterDisplay : server key does not match.');
          throw new SoapFault('Sender', 'The Server key you entered does not match with the server key at this address');
        }
        
        // Check the Length of the hardwareKey
        if (strlen($hardwareKey) > 40) {
	  syslog(LOG_INFO, 'RegisterDisplay : Hardware key is too long.');
          throw new SoapFault('Sender', 'The Hardware Key you sent was too long. Only 40 characters are allowed (SHA1).');
	}
	
        // Check in the database for this hardwareKey
        try {
	    syslog(LOG_INFO, 'RegisterDisplay , connecting PDO');
            $dbh = PDOConnect::init();
	    syslog(LOG_INFO, 'RegisterDisplay , connected.');
            $sth = $dbh->prepare('
        ...
        // Return an XML formatted stringcreate a new object
        ...
        if (count($result) == 0) {
      	    ...
      	    syslog(LOG_INFO, 'RegisterDisplay , hardwareKey is new');
            ...
            $jsonResult->status = 1;
	    $jsonResult->code = 'ADDED';
	    $jsonResult->message = 'Display added and is awaiting licensing approval from an Administrator.';
	    $jsonResult->version_instructions = $versionInstructions;

        }
        else {
      	    syslog(LOG_INFO, 'RegisterDisplay , hardwareKey is old');
            ...
            if ($row['licensed'] == 0) {
                ...
                $jsonResult->status = 2;
		$jsonResult->code = 'WAITING';
		$jsonResult->message = 'Display is awaiting licensing approval from an Administrator.';
            }
            else {
 		$jsonResult->status = 1;
		$jsonResult->code = 'READY';
		$jsonResult->message = 'Display is active and ready to start.';
		$jsonResult->version_instructions = $versionInstructions;
                ...
                try {
                    ...
                    $displayProfile->displayProfileIdXMDSJsonRpc = (empty($row['displayprofileid']) ? 0 : Kit::ValidateParam($row['displayprofileid'], _INT));
        ...
        syslog(LOG_INFO, 'returnXml : '.$returnXml);
        if (empty($jsonResult)) {
	  syslog(LOG_INFO, 'calling encode_json with json null ');
        } else {
	  syslog(LOG_INFO, 'calling encode_json with json not null ');
        }
//        $resultArray = objectToArray($jsonResult);
	syslog(LOG_INFO, 'calling encode_json ...');
        $returnJson = json_encode($jsonResult);
	syslog(LOG_INFO, 'calling LogBandwidth ...');
        $this->LogBandwidth($displayid, Bandwidth::$REGISTER, strlen($returnJson));

        Debug::LogEntry('audit', $returnJson, get_class(), __FUNCTION__);
	syslog(LOG_INFO, 'returning from registerDisplay');
        return $returnJson;
    }
/*
...
*/
	...
	syslog(LOG_INFO,'requiredFiles');
        ...
        try {
            ...
            foreach ($sth->fetchAll() as $row) {
                ...
                $jsonFile->type = $recordType;
                $jsonFile->id =   $id;
                $jsonFile->size = $fileSize;
                $jsonFile->md5 = $md5;
                $jsonFile->download = xmds;
                $jsonFile->path = $path;
                $jfiles.push($jsonFile);
            ...
            foreach($layoutInformation['regions'] as $region) {
                foreach($region['media'] as $media) {
                    if ($media['render'] == 'html' || $media['mediatype'] == 'ticker' || $media['mediatype'] == 'text' || $media['mediatype'] == 'datasetview' || $media['mediatype'] == 'webpage' || $media['mediatype'] == 'embedded') {
                        ...
                        $jsonFile = new stdClass();
			$jsonFile->type = 'resource';
			$jsonFile->id =  rand();

			$jsonFile->layoutid = $layoutId;
			$jsonFile->regionid = $region['regionid'];
			$jsonFile->mediaid = $media['mediaid'];
			$jsonFile->updated =  (isset($media['updated']) ? $media['updated'] : 0);
			$jfiles.push($jsonFile);
        ...
        $returnJson = json_encode($jsonResult);
        // Log Bandwidth
        $this->LogBandwidth($this->displayId, Bandwidth::$RF, strlen($returnJson));
        return $returnJson;
    }
     ...
     */
    function mediaInventory($version, $serverKey, $hardwareKey, $inventory)
    {
        // Sanitize
        $serverKey = Kit::ValidateParam($serverKey, _STRING);
        $hardwareKey = Kit::ValidateParam($hardwareKey, _STRING);
