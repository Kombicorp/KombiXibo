...
// gregm added
syslog(LOG_INFO, "after Config::Load()");
...
try {
    ...
    syslog(LOG_INFO, 'Database connection problem. ' . $e->getMessage());
    die('Database connection problem. ' . $e->getMessage());
}

syslog(LOG_INFO, "after PDOConnect::init()");

// create a database class instance (legacy)
$db = new database();

syslog(LOG_INFO, "after new database() connecting : ".$dbuser." , ".$dbpass);
...
syslog(LOG_INFO, "after date_default_timezone_set");
